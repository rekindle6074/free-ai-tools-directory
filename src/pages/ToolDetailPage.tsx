            ...
            <div className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              {user ? (
                ...
                <button
                  ...
                  aria-label={`Connect to visit ${tool.name}`}
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-2.5 px-8 py-3.5 bg-slate-900 text-white font-bold text-sm rounded-xl hover:bg-emerald-600 transition-all shadow-md shadow-slate-900/10"
                >
                  <Lock className="w-4 h-4 text-emerald-400" />
                  <span>Accès réservé aux membres inscrits</span>
                </button>
              )}
            </div>
          </motion.div>
          ...
          {relatedTools.length > 0 && (
            <div>
              ...
              <MemberToolsGuard 
                ...
                categoryTitle={tool.category}
                customMessage="La liste des outils similaires et leurs accès directs sont exclusivement réservés aux utilisateurs enregistrés. Les inscriptions publiques sont actuellement suspendues."
              >
